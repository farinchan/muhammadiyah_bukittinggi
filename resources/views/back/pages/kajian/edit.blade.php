    ...
    <div id="kt_app_content" class="app-content flex-column-fluid">
        <div id="kt_app_content_container" class="app-container container-xxl">
            <form id="kt_ecommerce_add_category_form" class="form d-flex flex-column flex-lg-row"
                action="{{ route('admin.kajian.update', $kajian->id) }}" method="POST" enctype="multipart/form-data">
                @method('PUT')
                @csrf
                        ...
                        <div class="card-body text-center pt-0">
                            <style>
                                .image-input-placeholder {
                                    background-image: url('@if($kajian->thumbnail) {{ Storage::url($kajian->thumbnail) }} @else {{ asset('back/media/svg/files/blank-image.svg') }} @endif');
                                }
                            </style>
                    ...
                    <div class="card card-flush py-4">
                        ...
                        <div class="card-body pt-0">
                            <select name="status" class="form-select mb-2" data-control="select2" data-hide-search="true"
                                ...
                                <option></option>
                                <option value="published" {{ $kajian->status == 'published' ? 'selected' : '' }}>Published</option>
                                <option value="draft" {{ $kajian->status == 'draft' ? 'selected' : '' }}>Draft</option>
                                <option value="archived" {{ $kajian->status == 'archived' ? 'selected' : '' }}>Archived</option>
                            </select>
                            @error('status')
                                <div class="text-danger fs-7">{{ $message }}</div>
                            @enderror
                            <div class="text-muted fs-7">
                                Set Status Kajian, <code>Published</code> untuk mempublikasikan Kajian, <code>Draft</code> untuk menyimpan Kajian sebagai draft, <code>Archived</code> untuk menyimpan Kajian sebagai arsip
                            </div>
                        </div>
                    </div>
                    <div class="card card-flush py-4">
                        ...
                        <div class="card-body pt-0">
                            <select name="user_id" class="form-select mb-2" data-control="select2" data-hide-search="true"
                                ...
                                @foreach ($users as $user)
                                    <option value="{{ $user->id }}" {{ $kajian->user_id == $user->id ? 'selected' : '' }}>
                                        {{ $user->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="card-body pt-0">
                            <div class="mb-10 fv-row">
                                ...
                                <input type="text" name="title" class="form-control mb-2"
                                    placeholder="Judul Pengumuman" value="{{ $kajian->title }}" required />
                                @error('title')
                                    <div class="text-danger fs-7">{{ $message }}</div>
                                @enderror
                            </div>
