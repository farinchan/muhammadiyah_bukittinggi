    ...
    <div id="kt_app_content" class="app-content flex-column-fluid">
        <div id="kt_app_content_container" class="app-container container-xxl">
            <form id="kt_ecommerce_add_category_form" class="form d-flex flex-column flex-lg-row"
                action="{{ route('admin.asset.store') }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="d-flex flex-column gap-7 gap-lg-10 w-100 w-lg-300px mb-7 me-lg-10">
                    ...
                    <div class="card card-flush py-4">
                        ...
                        <div class="card-body pt-0">
                            @if ($asset_type)
                                <input class="form-control mb-2" type="text" value="{{ $asset_type->name }}" readonly />
                                ...
                                <select name="asset_type_id" class="form-select mb-2" data-control="select2"
                                    ...
                                    @foreach ($list_asset_type as $type)
                                        <option value="{{ $type->id }}" @if (old('asset_type_id') == $type->id) selected @endif>
                                            {{ $type->name }} </option>
                                    @endforeach
                                </select>
                            @endif
                    ...
                    <div class="card card-flush py-4">
                        ...
                        <div class="card-body pt-0">
                            ...
                            <div class="mb-5">
                                ...
                                <textarea name="description" class="form-control mb-2" rows="5" placeholder="Deskripsi Tempat">{{ old('description') }}</textarea>
                                @error('description')
                                    <div class="text-danger fs-7">{{ $message }}</div>
                                @enderror
                            </div>
                    ...
                    <div class="card card-flush py-4">
                        ...
                        <div class="card-body pt-0">
                            <div class="mb-5">
                                ...
                                <textarea name="address" class="form-control mb-2" rows="5" placeholder="Alamat Tempat" required>{{ old('address') }}</textarea>
                                @error('address')
                                    <div class="text-danger fs-7">{{ $message }}</div>
                                @enderror
                            </div>
                            <div class="row mb-5">
                                <div class="col-md-6">
                                    ...
                                    <input type="text" name="latitude" class="form-control mb-2"
                                        placeholder="Latitude" id="latitude" value="{{ old('latitude') }}" readonly />
                                    @error('latitude')
                                        <div class="text-danger fs-7">{{ $message }}</div>
                                    @enderror
                                </div>
                                <div class="col-md-6">
                                    ...
                                    <input type="text" name="longitude" class="form-control mb-2"
                                        placeholder="Longitude" id="longitude" value="{{ old('longitude') }}"
                                        readonly />
                                    @error('longitude')
                                        <div class="text-danger fs-7">{{ $message }}</div>
                                    @enderror
                            ...
                            <div class="row mb-5">
                                ...
                                <div class="col-md-6">
                                    ...
                                    <input type="url" name="instagram" class="form-control mb-2"
                                        placeholder="Instagram Tempat" value="{{ old('instagram') }}" />
                                    @error('instagram')
                                        <div class="text-danger fs-7">{{ $message }}</div>
                                    @enderror
                                </div>
...
@section('scripts')
    ...
    <script>
        ...
        // Jika lokasi pengguna ditemukan, set marker ke lokasi tersebut
        ...
        // Jika lokasi pengguna tidak ditemukan, gunakan Bukittinggi sebagai fallback
        map.on('locationerror', function() {
            var bukittinggiLatLng = [-0.305441, 100.3692];
            marker.setLatLng(bukittinggiLatLng);
            map.setView(bukittinggiLatLng, 13);
            document.getElementById('latitude').value = bukittinggiLatLng[0];
            document.getElementById('longitude').value = bukittinggiLatLng[1];
            alert("Lokasi pengguna tidak dapat ditemukan. Menggunakan lokasi default di Bukittinggi.");
        });
    </script>
@endsection
